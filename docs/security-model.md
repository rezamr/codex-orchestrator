...
- **Windows verification:** npm/npx resolve to native Node plus the installed CLI script; batch files are not blindly launched or converted into concatenated shell commands. Unsupported wrappers produce actionable failed check evidence. Literal shell metacharacter arguments and synchronous launch errors have regression tests.
- **Desktop links:** the new thread-open channel accepts only an id, verifies saved metadata, and constructs the documented `codex://threads/<id>` route in main. Reserved new-chat routes, path/query injection, unknown threads, and arbitrary URLs are rejected. Generic external links remain HTTPS-allowlisted. Tests intercept OS opening rather than launching the real desktop app.
