...
The current `0.1.0-alpha.2` implementation includes the desktop project/job workflow, fake and Codex app-server providers, automatic Codex CLI discovery, read-only account/usage/history views across the main screens, discovered workspaces, explicit same-thread continuation with destination identity, official ChatGPT desktop thread links, durable jobs/schedules/recovery, verification-only retry, approvals, notifications, diagnostics, and simulated/guarded platform power adapters. Maintainer live Codex checks remain read-only; user-reported turns do not establish a completed controlled release test. No controlled real power-action test is claimed.
